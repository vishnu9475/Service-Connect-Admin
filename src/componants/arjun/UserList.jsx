    ...
    <div className='bg-white rounded-xl overflow-visible'>
      <table className='w-full text-sm min-w-[1000px]'>
        ...
        <tbody className="divide-y divide-gray-50">
          {users.map((u, index) => (
            <tr key={u.id} className={`transition-colors ${selectedUsers.includes(u.id) ? 'bg-[#F5F6FF]' : 'hover:bg-gray-50'}`}>
              <td className='px-6 py-4 text-center text-[#303972]'>
                <input 
                  type="checkbox" 
                  className="accent-[#4D44B5] h-4 w-4 rounded cursor-pointer"
              ...
              <td className='text-[#4D44B5] font-bold text-center'>{u.id}</td>
              <td className='text-[#A098AE] text-center'>{u.date}</td>
              <td className='text-[#303972] text-center font-bold'>{u.services}</td>
              <td className='text-[#303972] text-center'>{u.location}</td>
              <td>
              ...
              {/* --- LARGE WRAPPED STATUS BADGE --- */}
              {/* --- COMPACT STATUS BADGE --- */}
             <td className="text-center px-4">
                 <span className={`inline-flex items-center justify-center 
                            px-4 py-1.5           /* Smaller padding for a thinner bar */
                            min-w-[40px]         /* Reduced minimum width */
                            rounded-full 
                            text-xs font-normal   /* Normal font weight and smaller size */
                            text-white 
                       ${
                      u.status.toLowerCase() === "active" 
                           ? "bg-[#0EAB56]"  // Forest Green
                        : "bg-[#FF5B22]"  // Vibrant Orange
                       }`}>
                       {u.status}
               </span>
             </td>
              <td className="px-6 py-4 text-center">
                <div className="relative inline-block" ref={activeMenu === index ? menuRef : null}>
                  ...
                  {activeMenu === index && (
                    <div className={`absolute right-0 w-44 bg-white rounded-xl shadow-xl border border-gray-100 z-50 py-2 ${index >= users.length - 2 ? 'bottom-full mb-2' : 'mt-2'}`}>
