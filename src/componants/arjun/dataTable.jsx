...
<div className="p-4 flex justify-end border-b border-gray-100 bg-gray-50/50">
  <button
    ...
    className={`p-2 transition-all duration-200 ${
      hasSelection 
        ? "text-[#4D44B5] cursor-pointer hover:scale-110" 
        : "text-gray-300 cursor-not-allowed"
    }`}
    title={hasSelection ? `Delete selected (${selectedIds.length})` : "Select rows to delete"}
  >
    <FiTrash2 size={20} />
  </button>
</div>
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-6 py-4 text-sm">
        <p className="text-gray-500">
          Showing <span className="font-medium text-gray-700">{totalData === 0 ? 0 : startIndex + 1}</span>–
        ...
        <div className="flex items-center gap-2">
          ...
          >
            <div className="w-0 h-0 border-t-[6px] border-b-[6px] border-r-[10px] border-t-transparent border-b-transparent border-r-gray-400 group-hover:border-r-indigo-600 transition-colors" />
          </button>
          ...
          <button
            ...
            className="w-8 h-8 flex items-center justify-center disabled:opacity-30 group"
          >
            <div className="w-0 h-0 border-t-[6px] border-b-[6px] border-l-[10px] border-t-transparent border-b-transparent border-l-gray-400 group-hover:border-l-indigo-600 transition-colors" />
          </button>
        </div>
      </div>
    </div>
