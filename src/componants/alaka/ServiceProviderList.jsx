...
function ServiceProviderList() {
  ...
  if (["Active", "Notactive", "Verified", "Not Verified"].includes(filter)) {
    if (filter === "Verified") {
      return match && u.status !== "Not Verified";
    }
    return match && u.status === filter;
  }
...
<div ref={filterRef} className="relative">
  <button
    onClick={() => setFilterOpen((prev) => !prev)}
    className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-indigo-500 text-indigo-600 text-sm bg-white"
  >
    {filter}
    <ChevronDownIcon
...
/>
  ...
  {filterOpen && (
    <div className="absolute right-0 mt-2 w-44 bg-white border rounded-xl shadow-lg z-50 transition-all duration-200 animate-fadeIn">
      {["All", "Active", "Notactive", "Verified", "Not Verified"].map((opt) => (
        <button
          key={opt}
          onClick={() => {
  ...
  <div className="flex gap-2">
    ...
    >
      <EnvelopeIcon className="w-4 h-4 text-indigo-600" />
                ...
                <td className="py-4 px-3">
                  <span
                    className={`inline-flex w-[90px] justify-center py-1 rounded-full text-white text-xs ${user.statusColor}`}
                  >
                    {user.status}
                  </span>
                </td>

                <td ref={actionRef} className="px-3 py-4 text-center relative">
                  <EllipsisHorizontalIcon
                    onClick={() =>
                      setOpenMenuId(openMenuId === user.id ? null : user.id)
                    }
                    className="w-6 h-5 text-black-600 cursor-pointer mx-auto"
                  />
                  {openMenuId === user.id && (
                    <div className="absolute right-6 top-10 w-32 bg-white border rounded shadow text-xs z-20">
                      <button
